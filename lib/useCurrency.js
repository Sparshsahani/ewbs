...
const IPIFY_URL    = "https://api.ipify.org?format=json";  // step 1: get real public IP
const IPINFO_URL   = "https://api.ipinfo.io/lite/";        // step 2: ipinfo with that IP
const CURRENCY_URL = "https://newcrm.ewbsbusiness.ae/api/v1/Currency";
...
const CACHE_KEY     = "ewbs_currency_v5"; // full result, expires after 24 h
    ...
    async function init() {
      ...
      try {
        ...
        if (!countryCode) {
          ...
          const ipRes = await fetch(`${IPINFO_URL}${ip}?token=b0beada749205e`);
          if (ipRes.ok) {
        ...
        const result = {
          ...
          rate: targetRecord ? targetRecord.amt : 1,
          inrRate: inrRecord ? inrRecord.amt : DEFAULT_DATA.inrRate,
          baseCurrency: "AED",
          cachedAt: Date.now(),
        };
