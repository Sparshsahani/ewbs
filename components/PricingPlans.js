...
import { useCurrency, convertPriceFromINR, convertPriceFromAED } from "@/lib/useCurrency";
...
export default function PricingPlans({
  ...
  const { loading, currencyCode, rate, inrRate } = useCurrency();
  ...
  // Helper: convert a plan's stored price to the user's currency
  function displayPrice(plan) {
    if (loading) return { amount: "...", code: "" };
    if (plan.currency === "AED") {
      // Price stored in AED → multiply by rate (1 AED = rate target-currency)
      return convertPriceFromAED(plan.price, currencyCode, rate, inrRate);
    }
    // Default: price stored in INR → INR → AED → target
    return convertPriceFromINR(plan.price, inrRate, currencyCode, rate);
  }

  // ── onlyCTA early-return (safe now that hooks are above) ───────────────
  if (onlyCTA) {
    return (
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 lg:px-6 xl:px-20 2xl:px-8">
        {/* Heading */}
        <p className="mb-10 text-start xl:text-xl 2xl:text-2xl ">{heading}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-14">
          {plans.map((plan, i) => {
            ...
            const { amount: dispAmount, code: dispCode } = displayPrice(plan);
            return (
              <div
                key={i}
            ...
            >
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  {/* price */}
                  {(() => {
                    const { amount: mAmount, code: mCode } =
